/**
 * Asian countries data for database seeding
 */

import { CountryData } from "./countries-data";

export const asianCountriesData: CountryData[] = [
  {
    code: "AFG",
    continent: "Asia",
    region: "Southern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Afghanistan", description: "Islamic Emirate of Afghanistan" },
      { locale: "ar", name: "أفغانستان", description: "إمارة أفغانستان الإسلامية" },
      { locale: "es", name: "Afganistán", description: "Emirato Islámico de Afganistán" },
      { locale: "fr", name: "Afghanistan", description: "Émirat islamique d'Afghanistan" },
    ],
  },
  {
    code: "BGD",
    continent: "Asia",
    region: "Southern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Bangladesh", description: "People's Republic of Bangladesh" },
      { locale: "ar", name: "بنغلاديش", description: "جمهورية بنغلاديش الشعبية" },
      { locale: "es", name: "Bangladesh", description: "República Popular de Bangladesh" },
      { locale: "fr", name: "Bangladesh", description: "République populaire du Bangladesh" },
    ],
  },
  {
    code: "BTN",
    continent: "Asia",
    region: "Southern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Bhutan", description: "Kingdom of Bhutan" },
      { locale: "ar", name: "بوتان", description: "مملكة بوتان" },
      { locale: "es", name: "Bután", description: "Reino de Bután" },
      { locale: "fr", name: "Bhoutan", description: "Royaume du Bhoutan" },
    ],
  },
  {
    code: "BRN",
    continent: "Asia",
    region: "South-Eastern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Brunei", description: "Brunei Darussalam" },
      { locale: "ar", name: "بروناي", description: "بروناي دار السلام" },
      { locale: "es", name: "Brunéi", description: "Brunéi Darussalam" },
      { locale: "fr", name: "Brunéi", description: "Brunéi Darussalam" },
    ],
  },
  {
    code: "KHM",
    continent: "Asia",
    region: "South-Eastern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Cambodia", description: "Kingdom of Cambodia" },
      { locale: "ar", name: "كمبوديا", description: "مملكة كمبوديا" },
      { locale: "es", name: "Camboya", description: "Reino de Camboya" },
      { locale: "fr", name: "Cambodge", description: "Royaume du Cambodge" },
    ],
  },
  {
    code: "CHN",
    continent: "Asia",
    region: "Eastern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "China", description: "People's Republic of China" },
      { locale: "ar", name: "الصين", description: "جمهورية الصين الشعبية" },
      { locale: "es", name: "China", description: "República Popular China" },
      { locale: "fr", name: "Chine", description: "République populaire de Chine" },
    ],
  },
  {
    code: "TLS",
    continent: "Asia",
    region: "South-Eastern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "East Timor", description: "Democratic Republic of Timor-Leste" },
      { locale: "ar", name: "تيمور الشرقية", description: "جمهورية تيمور الشرقية الديمقراطية" },
      { locale: "es", name: "Timor Oriental", description: "República Democrática de Timor Oriental" },
      { locale: "fr", name: "Timor oriental", description: "République démocratique du Timor oriental" },
    ],
  },
  {
    code: "GEO",
    continent: "Asia",
    region: "Western Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Georgia", description: "Georgia" },
      { locale: "ar", name: "جورجيا", description: "جورجيا" },
      { locale: "es", name: "Georgia", description: "Georgia" },
      { locale: "fr", name: "Géorgie", description: "Géorgie" },
    ],
  },
  {
    code: "IND",
    continent: "Asia",
    region: "Southern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "India", description: "Republic of India" },
      { locale: "ar", name: "الهند", description: "جمهورية الهند" },
      { locale: "es", name: "India", description: "República de la India" },
      { locale: "fr", name: "Inde", description: "République de l'Inde" },
    ],
  },
  {
    code: "IDN",
    continent: "Asia",
    region: "South-Eastern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Indonesia", description: "Republic of Indonesia" },
      { locale: "ar", name: "إندونيسيا", description: "جمهورية إندونيسيا" },
      { locale: "es", name: "Indonesia", description: "República de Indonesia" },
      { locale: "fr", name: "Indonésie", description: "République d'Indonésie" },
    ],
  },
  {
    code: "IRN",
    continent: "Asia",
    region: "Southern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Iran", description: "Islamic Republic of Iran" },
      { locale: "ar", name: "إيران", description: "جمهورية إيران الإسلامية" },
      { locale: "es", name: "Irán", description: "República Islámica de Irán" },
      { locale: "fr", name: "Iran", description: "République islamique d'Iran" },
    ],
  },
  {
    code: "IRQ",
    continent: "Asia",
    region: "Western Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Iraq", description: "Republic of Iraq" },
      { locale: "ar", name: "العراق", description: "جمهورية العراق" },
      { locale: "es", name: "Irak", description: "República de Irak" },
      { locale: "fr", name: "Irak", description: "République d'Irak" },
    ],
  },
  {
    code: "ISR",
    continent: "Asia",
    region: "Western Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Israel", description: "State of Israel" },
      { locale: "ar", name: "إسرائيل", description: "دولة إسرائيل" },
      { locale: "es", name: "Israel", description: "Estado de Israel" },
      { locale: "fr", name: "Israël", description: "État d'Israël" },
    ],
  },
  {
    code: "JPN",
    continent: "Asia",
    region: "Eastern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Japan", description: "Japan" },
      { locale: "ar", name: "اليابان", description: "اليابان" },
      { locale: "es", name: "Japón", description: "Japón" },
      { locale: "fr", name: "Japon", description: "Japon" },
    ],
  },
  {
    code: "JOR",
    continent: "Asia",
    region: "Western Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Jordan", description: "Hashemite Kingdom of Jordan" },
      { locale: "ar", name: "الأردن", description: "المملكة الأردنية الهاشمية" },
      { locale: "es", name: "Jordania", description: "Reino Hachemita de Jordania" },
      { locale: "fr", name: "Jordanie", description: "Royaume hachémite de Jordanie" },
    ],
  },
  {
    code: "KAZ",
    continent: "Asia",
    region: "Central Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Kazakhstan", description: "Republic of Kazakhstan" },
      { locale: "ar", name: "كازاخستان", description: "جمهورية كازاخستان" },
      { locale: "es", name: "Kazajistán", description: "República de Kazajistán" },
      { locale: "fr", name: "Kazakhstan", description: "République du Kazakhstan" },
    ],
  },
  {
    code: "KWT",
    continent: "Asia",
    region: "Western Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Kuwait", description: "State of Kuwait" },
      { locale: "ar", name: "الكويت", description: "دولة الكويت" },
      { locale: "es", name: "Kuwait", description: "Estado de Kuwait" },
      { locale: "fr", name: "Koweït", description: "État du Koweït" },
    ],
  },
  {
    code: "KGZ",
    continent: "Asia",
    region: "Central Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Kyrgyzstan", description: "Kyrgyz Republic" },
      { locale: "ar", name: "قيرغيزستان", description: "جمهورية قيرغيزستان" },
      { locale: "es", name: "Kirguistán", description: "República Kirguisa" },
      { locale: "fr", name: "Kirghizistan", description: "République kirghize" },
    ],
  },
  {
    code: "LAO",
    continent: "Asia",
    region: "South-Eastern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Laos", description: "Lao People's Democratic Republic" },
      { locale: "ar", name: "لاوس", description: "جمهورية لاوس الديمقراطية الشعبية" },
      { locale: "es", name: "Laos", description: "República Democrática Popular Lao" },
      { locale: "fr", name: "Laos", description: "République démocratique populaire lao" },
    ],
  },
  {
    code: "LBN",
    continent: "Asia",
    region: "Western Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Lebanon", description: "Lebanese Republic" },
      { locale: "ar", name: "لبنان", description: "الجمهورية اللبنانية" },
      { locale: "es", name: "Líbano", description: "República Libanesa" },
      { locale: "fr", name: "Liban", description: "République libanaise" },
    ],
  },
  {
    code: "MYS",
    continent: "Asia",
    region: "South-Eastern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Malaysia", description: "Malaysia" },
      { locale: "ar", name: "ماليزيا", description: "ماليزيا" },
      { locale: "es", name: "Malasia", description: "Malasia" },
      { locale: "fr", name: "Malaisie", description: "Malaisie" },
    ],
  },
  {
    code: "MDV",
    continent: "Asia",
    region: "Southern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Maldives", description: "Republic of Maldives" },
      { locale: "ar", name: "جزر المالديف", description: "جمهورية جزر المالديف" },
      { locale: "es", name: "Maldivas", description: "República de las Maldivas" },
      { locale: "fr", name: "Maldives", description: "République des Maldives" },
    ],
  },
  {
    code: "MNG",
    continent: "Asia",
    region: "Eastern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Mongolia", description: "Mongolia" },
      { locale: "ar", name: "منغوليا", description: "منغوليا" },
      { locale: "es", name: "Mongolia", description: "Mongolia" },
      { locale: "fr", name: "Mongolie", description: "Mongolie" },
    ],
  },
  {
    code: "MMR",
    continent: "Asia",
    region: "South-Eastern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Myanmar", description: "Republic of the Union of Myanmar" },
      { locale: "ar", name: "ميانمار", description: "جمهورية اتحاد ميانمار" },
      { locale: "es", name: "Myanmar", description: "República de la Unión de Myanmar" },
      { locale: "fr", name: "Myanmar", description: "République de l'Union du Myanmar" },
    ],
  },
  {
    code: "NPL",
    continent: "Asia",
    region: "Southern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Nepal", description: "Federal Democratic Republic of Nepal" },
      { locale: "ar", name: "نيبال", description: "جمهورية نيبال الديمقراطية الاتحادية" },
      { locale: "es", name: "Nepal", description: "República Federal Democrática de Nepal" },
      { locale: "fr", name: "Népal", description: "République fédérale démocratique du Népal" },
    ],
  },
  {
    code: "PRK",
    continent: "Asia",
    region: "Eastern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "North Korea", description: "Democratic People's Republic of Korea" },
      { locale: "ar", name: "كوريا الشمالية", description: "جمهورية كوريا الديمقراطية الشعبية" },
      { locale: "es", name: "Corea del Norte", description: "República Popular Democrática de Corea" },
      { locale: "fr", name: "Corée du Nord", description: "République populaire démocratique de Corée" },
    ],
  },
  {
    code: "OMN",
    continent: "Asia",
    region: "Western Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Oman", description: "Sultanate of Oman" },
      { locale: "ar", name: "عُمان", description: "سلطنة عُمان" },
      { locale: "es", name: "Omán", description: "Sultanato de Omán" },
      { locale: "fr", name: "Oman", description: "Sultanat d'Oman" },
    ],
  },
  {
    code: "PAK",
    continent: "Asia",
    region: "Southern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Pakistan", description: "Islamic Republic of Pakistan" },
      { locale: "ar", name: "باكستان", description: "جمهورية باكستان الإسلامية" },
      { locale: "es", name: "Pakistán", description: "República Islámica de Pakistán" },
      { locale: "fr", name: "Pakistan", description: "République islamique du Pakistan" },
    ],
  },
  {
    code: "PSE",
    continent: "Asia",
    region: "Western Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Palestine", description: "State of Palestine" },
      { locale: "ar", name: "فلسطين", description: "دولة فلسطين" },
      { locale: "es", name: "Palestina", description: "Estado de Palestina" },
      { locale: "fr", name: "Palestine", description: "État de Palestine" },
    ],
  },
  {
    code: "PHL",
    continent: "Asia",
    region: "South-Eastern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Philippines", description: "Republic of the Philippines" },
      { locale: "ar", name: "الفلبين", description: "جمهورية الفلبين" },
      { locale: "es", name: "Filipinas", description: "República de Filipinas" },
      { locale: "fr", name: "Philippines", description: "République des Philippines" },
    ],
  },
  {
    code: "QAT",
    continent: "Asia",
    region: "Western Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Qatar", description: "State of Qatar" },
      { locale: "ar", name: "قطر", description: "دولة قطر" },
      { locale: "es", name: "Catar", description: "Estado de Catar" },
      { locale: "fr", name: "Qatar", description: "État du Qatar" },
    ],
  },
  {
    code: "SAU",
    continent: "Asia",
    region: "Western Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Saudi Arabia", description: "Kingdom of Saudi Arabia" },
      { locale: "ar", name: "السعودية", description: "المملكة العربية السعودية" },
      { locale: "es", name: "Arabia Saudí", description: "Reino de Arabia Saudí" },
      { locale: "fr", name: "Arabie saoudite", description: "Royaume d'Arabie saoudite" },
    ],
  },
  {
    code: "SGP",
    continent: "Asia",
    region: "South-Eastern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Singapore", description: "Republic of Singapore" },
      { locale: "ar", name: "سنغافورة", description: "جمهورية سنغافورة" },
      { locale: "es", name: "Singapur", description: "República de Singapur" },
      { locale: "fr", name: "Singapour", description: "République de Singapour" },
    ],
  },
  {
    code: "KOR",
    continent: "Asia",
    region: "Eastern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "South Korea", description: "Republic of Korea" },
      { locale: "ar", name: "كوريا الجنوبية", description: "جمهورية كوريا" },
      { locale: "es", name: "Corea del Sur", description: "República de Corea" },
      { locale: "fr", name: "Corée du Sud", description: "République de Corée" },
    ],
  },
  {
    code: "LKA",
    continent: "Asia",
    region: "Southern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Sri Lanka", description: "Democratic Socialist Republic of Sri Lanka" },
      { locale: "ar", name: "سريلانكا", description: "جمهورية سريلانكا الديمقراطية الاشتراكية" },
      { locale: "es", name: "Sri Lanka", description: "República Democrática Socialista de Sri Lanka" },
      { locale: "fr", name: "Sri Lanka", description: "République démocratique socialiste du Sri Lanka" },
    ],
  },
  {
    code: "SYR",
    continent: "Asia",
    region: "Western Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Syria", description: "Syrian Arab Republic" },
      { locale: "ar", name: "سوريا", description: "الجمهورية العربية السورية" },
      { locale: "es", name: "Siria", description: "República Árabe Siria" },
      { locale: "fr", name: "Syrie", description: "République arabe syrienne" },
    ],
  },
  {
    code: "TJK",
    continent: "Asia",
    region: "Central Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Tajikistan", description: "Republic of Tajikistan" },
      { locale: "ar", name: "طاجيكستان", description: "جمهورية طاجيكستان" },
      { locale: "es", name: "Tayikistán", description: "República de Tayikistán" },
      { locale: "fr", name: "Tadjikistan", description: "République du Tadjikistan" },
    ],
  },
  {
    code: "THA",
    continent: "Asia",
    region: "South-Eastern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Thailand", description: "Kingdom of Thailand" },
      { locale: "ar", name: "تايلاند", description: "مملكة تايلاند" },
      { locale: "es", name: "Tailandia", description: "Reino de Tailandia" },
      { locale: "fr", name: "Thaïlande", description: "Royaume de Thaïlande" },
    ],
  },
  {
    code: "TUR",
    continent: "Asia",
    region: "Western Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Turkey", description: "Republic of Turkey" },
      { locale: "ar", name: "تركيا", description: "جمهورية تركيا" },
      { locale: "es", name: "Turquía", description: "República de Turquía" },
      { locale: "fr", name: "Turquie", description: "République de Turquie" },
    ],
  },
  {
    code: "TKM",
    continent: "Asia",
    region: "Central Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Turkmenistan", description: "Turkmenistan" },
      { locale: "ar", name: "تركمانستان", description: "تركمانستان" },
      { locale: "es", name: "Turkmenistán", description: "Turkmenistán" },
      { locale: "fr", name: "Turkménistan", description: "Turkménistan" },
    ],
  },
  {
    code: "ARE",
    continent: "Asia",
    region: "Western Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "United Arab Emirates", description: "United Arab Emirates" },
      { locale: "ar", name: "الإمارات العربية المتحدة", description: "دولة الإمارات العربية المتحدة" },
      { locale: "es", name: "Emiratos Árabes Unidos", description: "Emiratos Árabes Unidos" },
      { locale: "fr", name: "Émirats arabes unis", description: "Émirats arabes unis" },
    ],
  },
  {
    code: "UZB",
    continent: "Asia",
    region: "Central Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Uzbekistan", description: "Republic of Uzbekistan" },
      { locale: "ar", name: "أوزبكستان", description: "جمهورية أوزبكستان" },
      { locale: "es", name: "Uzbekistán", description: "República de Uzbekistán" },
      { locale: "fr", name: "Ouzbékistan", description: "République d'Ouzbékistan" },
    ],
  },
  {
    code: "VNM",
    continent: "Asia",
    region: "South-Eastern Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Vietnam", description: "Socialist Republic of Vietnam" },
      { locale: "ar", name: "فيتنام", description: "جمهورية فيتنام الاشتراكية" },
      { locale: "es", name: "Vietnam", description: "República Socialista de Vietnam" },
      { locale: "fr", name: "Vietnam", description: "République socialiste du Vietnam" },
    ],
  },
  {
    code: "YEM",
    continent: "Asia",
    region: "Western Asia",
    isActive: true,
    translations: [
      { locale: "en", name: "Yemen", description: "Republic of Yemen" },
      { locale: "ar", name: "اليمن", description: "جمهورية اليمن" },
      { locale: "es", name: "Yemen", description: "República de Yemen" },
      { locale: "fr", name: "Yémen", description: "République du Yémen" },
    ],
  },
];