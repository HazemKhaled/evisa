  ...
  const cachedFn = unstable_cache(
    async (): Promise<CountryWithI18n[]> => {
      try {
        ...
        console.warn(
          `Failed to get all countries for locale ${locale}:`,
          error
        );
        return [];
      }
    },
    ["all-countries", locale],
    { tags: ["countries", `countries-${locale}`], revalidate: 86400 }
  );
 ...
 */
export const getDestinationContinents = unstable_cache(
  async (_locale?: string): Promise<string[]> => {
    try {
      const db = getDb();
  ...
  ["continents"], // Cache key: [continents] - continent list is global
  { tags: ["destinations", "continents"], revalidate: 86400 }
);
...
export async function getDestinationDetails(
  ...
  const cachedFn = unstable_cache(
    async (): Promise<DestinationWithVisaTypes | null> => {
      ...
      if (passportCode) {
        ...
        if (!validatedPassportCode) {
          console.warn(
            `Invalid passport code provided: ${passportCode}, ignoring`
          );
        }
      }
    ...
    ["destination", destinationCode, locale],
    {
      tags: [
        "destinations",
