...
**Replace N+1 Pattern with Single Optimized Query**

```sql
-- New single query approach using JOINs and aggregations
SELECT
  c.id, c.code, c.name,
...
ORDER BY
  CASE WHEN ? = 'alphabetical' THEN ci.name END ASC,
  CASE WHEN ? = 'processing_time' THEN AVG(vt.processingTime) END ASC,
  CASE WHEN ? = 'visa_fee' THEN MIN(vt.fee) END ASC
LIMIT ?
```
...
CREATE INDEX idx_countries_i18n_locale_lookup ON countriesI18n(countryId, locale);
```
...
- Replace `Promise.all()` N+1 pattern with single JOIN query
- Move aggregations to database level (COUNT, AVG, MIN)
- Test query performance in development

**Subtask 1.2**: Add database indexes

- Create composite indexes for frequent lookups
- Optimize existing indexes for new query patterns
...
- Test query execution times with large datasets
- Validate index effectiveness
- Ensure query plans are optimal
...
- Leverage Drizzle's JOIN syntax for complex queries
